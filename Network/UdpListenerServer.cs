...
namespace IRAPROM.MyCore.MyNetwork
{
    public class UdpListenerServer
    {
...
#else
            if (IsSentByServer(remoteIpEndPoint))
            {
                return;
            }
#endif
        ...
        public static void ParseResponse(byte[] response, IPEndPoint remoteIpEndPoint)
        {
            ...
            {
                var responseMonopanel = DeviceFindAnswerNetworkInf.ParseOldImpulseResponse(response); // (PC 600MK 21.3 без пипки) (PC 4400MK 41.0 с пипкой) 
            ...
            try
            {
                if (Constants.FindAnswerCodes.Contains(rec.command))
                {
                    ...
                    else
                    {
                        ...
                        Validator.FoundDevices.Add(device);                                                         // FOR PassAlarmSimulator only!
                        MyARM.Instance.AddedDevicesAddForValidatorOnly(device.MAC, rec.MetDetector);                // FOR PassAlarmSimulator only!
                    }
                }
                else
                {
