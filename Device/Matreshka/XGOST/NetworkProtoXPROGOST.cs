...
{
    public class NetworkProtoXPROGOST : INetworkProtoDual, IDisposable
    {
        [JsonProperty]
...
//            Console.WriteLine($"Reading {Ip}:{_port} Header {Constants.FrameSequenceOffset} bytes...");
            ...
            var nRead = Stream.Read(headerBytes, 0, Constants.FrameSequenceOffset);

            if (nRead != Constants.FrameSequenceOffset)
            {
#if DEBUGG
                Console.WriteLine($"Reading {Ip}:{_port} XPROGOST header first {Constants.FrameSequenceOffset} bytes error!!!");
#endif
                return Array.Empty<byte>();
            }

            var frameLength = headerBytes[Constants.DataLengthOffset];
